    ...
    try:
        ...
        try:
            with open(user_base_file, 'r', encoding='UTF-8') as f:
                users = json.load(f)
                fast_key = users[str(user_id)]['class']
        except: None
        ...
        if text in forms['1']:
            ...
            # Формирование текста из времени
            times_list = ''
            if not lessons['Время 1']:
                vk.messages.send(
                    user_id=user_id,
                    random_id=random.getrandbits(64),
            ...
            les_time = 1
            for lesson in lessons['Время 1']:
                times_list += str(les_time) + '. ' + lesson.upper() + '\n'
                les_time += 1
            ...
            vk.messages.send(
                user_id=user_id,
                random_id=random.getrandbits(64),
                message='Red_C0mmisar v3.4\nПо предложениям и техническим вопросам обращайтесь:\nhttps://vk.com/redcommisar_helper',
            )
            ...
            vk.messages.send(
                user_id=user_id,
                random_id=random.getrandbits(64),
                message='/setfast {ваш класс} - добавить кнопку быстрого набора без рассылки\n'
                        '/setfast {ваш класс} update - присылать расписание когда оно обновляеться\n'
            ...
            try:
                ...
                if form:
                    with open(user_base_file, 'r', encoding='UTF-8') as f:
                        users = json.load(f)
                        users[str(user_id)] = {'id' : user_id, 'class' : form, 'date':'', 'up': 'update' in text}
            ...
            with open(user_base_file, 'r', encoding='UTF-8') as f:
                ...
                if str(user_id) in users:
                    users[str(user_id)]['date'] = lessons['date']
